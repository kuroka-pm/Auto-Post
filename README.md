...
exe 版のダウンロードとセットアップガイドは note 記事で提供しています。

📝 **[note 記事はこちら → kuroka_pm](https://note.com/kuroka_pm)**

> コードを読まずに、ダウンロード → 起動 → 設定だけで使えます。
> API キーの取得手順もスクリーンショット付きで解説しています。
...
- 本ツールの使用により生じたいかなる損害（アカウント凍結・API 利用料金・投稿内容に起因するトラブル等）について、開発者は一切の責任を負いません
- SNS の自動投稿は各プラットフォームの利用規約に抵触する可能性があります。**ご自身の責任でご利用ください**
- AI（Gemini）が生成した文章の内容・正確性について、開発者は保証しません
- API の利用には別途費用が発生する場合があります（[必要な API キー](#-必要な-api-キー) を参照）
